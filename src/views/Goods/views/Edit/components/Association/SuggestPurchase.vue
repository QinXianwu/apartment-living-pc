<template>
  <div class="SuggestPurchase">
    <div class="title">推荐购买</div>
    <div class="content">
      <el-form ref="form" :model="formData" label-width="120px">
        <el-form-item label="推荐购买">
...
export default {
  ...
  methods: {
    init() {
      const data = this.productInfo?.recommendProductList?.length
        ? this.productInfo.recommendProductList
        : [];
      data.forEach((item) => {
    ...
    async getQuery() {
      ...
      return new Promise(async (resolve) => {
        ...
        resolve({
          recommendProductList: this.list.map((item) => ({
